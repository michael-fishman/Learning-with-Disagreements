...
def main(dataset_name):
    columns_list = ['bayesian_vectors', 'original_distribution', 'crowdtruth_vectors']
    ...
    heatmap_data = {
        "Accuracy": pd.DataFrame(index=columns_list, columns=columns_list),
        "F1 Score": pd.DataFrame(index=columns_list, columns=columns_list),
        "CT F1 Score": pd.DataFrame(index=columns_list, columns=columns_list),
        "Cross-Entropy Loss": pd.DataFrame(index=columns_list, columns=columns_list),
        "Jensen-Shannon Divergence": pd.DataFrame(index=columns_list, columns=columns_list),
        "KL Divergence": pd.DataFrame(index=columns_list, columns=columns_list)
    }

    # Store loss per epoch for plotting
    epoch_losses = {f'{train_col}_{test_col}': {'train': [], 'test': []} for train_col, test_col in product(columns_list, repeat=2)}

    # Iterate over all combinations of columns for training and testing
    for train_col, test_col in product(columns_list, repeat=2):
        print(f'Training on {train_col}, Testing on {test_col}')

        # Use specific columns for train and test
        train_df_col = train_df[[train_col]].dropna()
        test_df_col = test_df[[test_col]].dropna()
        train_df_col = train_df_col.rename(columns={train_col: 'label'})
        test_df_col = test_df_col.rename(columns={test_col: 'label'})

        # Filter out rows with all zeros in the label column
        train_df_col = train_df_col[~train_df_col['label'].str.contains(r'\[0\.0, 0\.0, 0\.0\]')]
        test_df_col = test_df_col[~test_df_col['label'].str.contains(r'\[0\.0, 0\.0, 0\.0\]')]

        tokenizer = DebertaV2Tokenizer.from_pretrained('microsoft/deberta-v3-base')
        model = DebertaV2Model.from_pretrained('microsoft/deberta-v3-base').to(device)

        train_annotation_distributions = list_of_strings_to_tensor(train_df_col['label'].tolist(), device=device)
        test_annotation_distributions = list_of_strings_to_tensor(test_df_col['label'].tolist(), device=device)
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(0, len(train_df_col), batch_size):
                batch_texts = train_df_col['label'].iloc[i:i + batch_size].tolist()
            ...
            epoch_losses[f'{train_col}_{test_col}']['train'].append(epoch_train_loss)
            ...
            with torch.no_grad():
                for i in range(0, len(test_df_col), batch_size):
                    batch_texts = test_df_col['label'].iloc[i:i + batch_size].tolist()
            ...
            epoch_losses[f'{train_col}_{test_col}']['test'].append(epoch_test_loss)
        ...
        heatmap_data["Accuracy"].loc[test_col, train_col] = test_accuracy
        heatmap_data["F1 Score"].loc[test_col, train_col] = test_f1
        heatmap_data["CT F1 Score"].loc[test_col, train_col] = test_ct_f1
        heatmap_data["Cross-Entropy Loss"].loc[test_col, train_col] = test_ce_loss
        heatmap_data["Jensen-Shannon Divergence"].loc[test_col, train_col] = test_jsd
        heatmap_data["KL Divergence"].loc[test_col, train_col] = test_kl_div
    ...
    for ax, (metric, data) in zip(axes[:-1], heatmap_data.items()):
        # Rename the columns of data to be more beautiful
        data.columns = ['Bayesian Vectors', 'Original Distribution', 'Crowdtruth Vectors']
    ...
    for ax, (combination, losses) in zip(axes, epoch_losses.items()):
        ax.plot(range(1, len(losses['train']) + 1), losses['train'], marker='o', linestyle='-', color='b', label='Train Loss')
        ax.plot(range(1, len(losses['test']) + 1), losses['test'], marker='o', linestyle='-', color='r', label='Test Loss')
